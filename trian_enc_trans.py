...
# Prepare MNIST data
# data_train = crypten.load("./datas/alice_train.pth")
...
# dummy_input = torch.empty(batch_size, 1, 27, 27)
# dummy_input = torch.empty(batch_size, 1, 28, 28)
...
def train_model(model, train_loader, test_loader, epochs, learning_rate):
    ...
    for epoch in range(epochs):
        ...
        torch.save(model, 'model/alexnet_.pth')
    t1 = time.time()
...
if __name__ == '__main__':
    ...
    # test model
    model = torch.load("model/model081606_ep3_bt512_lr0.1_normal_77.pth")
    print(model)
    t0 = time.time()
